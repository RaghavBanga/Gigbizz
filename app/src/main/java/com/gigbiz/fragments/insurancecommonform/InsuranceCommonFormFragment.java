package com.gigbiz.fragments.insurancecommonform;
...
import com.gigbiz.databinding.FragmentInsuranceCommonFormBinding;
...
public class InsuranceCommonFormFragment extends Fragment {

    FragmentInsuranceCommonFormBinding binding;
...
//    File compressedPanCardPdf;
    ArrayList<String> interestedInsurance;
    public String value,type_list;


    public InsuranceCommonFormFragment() {
        // Required empty public constructor
    }

    public static InsuranceCommonFormFragment newInstance(String param1, String param2) {
        InsuranceCommonFormFragment fragment = new InsuranceCommonFormFragment();
        return fragment;
    }
...
//                        builder.show();
    ...
    private void getselectedCheckBox() {
        ...
        if (binding.healthinsurance.isChecked()){
            interestedInsurance.add("Health Insurance");
        }
        if (binding.carinsurance.isChecked()){
            interestedInsurance.add("Car Insurance");
        }
        if (binding.lifeinsurance.isChecked()){
            interestedInsurance.add("Life Insurance");
        }
    }
...
//                }
//                else {
                Call<InsuranceCommonResponse> call = RetrofitClient.getInstance().getApi().getInsuranceCommonResponse(
                        new InsuranceCommonRequest(
                ...
                call.enqueue(new Callback<InsuranceCommonResponse>() {
                    @Override
                    public void onResponse(Call<InsuranceCommonResponse> call, Response<InsuranceCommonResponse> response) {
                        InsuranceCommonResponse submitFlipKartFirstResponse = response.body();
...
//                            EventBus.getDefault().post(new MessageEvent(submitFlipKartFirstResponse.getReportId()));
                    ...
                    @Override
                    public void onFailure(Call<InsuranceCommonResponse> call, Throwable t) {
                        try {
...
//                }
    ...
    private String getInterestedList() {
        getselectedCheckBox();
        StringBuffer output = new StringBuffer();
        output.append("");
        for (String a: interestedInsurance) {
            output.append(a+",");
        }
