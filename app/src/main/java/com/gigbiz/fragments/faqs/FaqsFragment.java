package com.gigbiz.fragments.faqs;
...
import com.gigbiz.databinding.FragmentFaqsBinding;
        ...
        for (int i = 0; i < list.size(); i++) {
            if (value.equals(list.get(i).getProjectId())) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    binding.faqs.setText(Html.fromHtml(list.get(i).getFaq(), Html.FROM_HTML_MODE_COMPACT));
                } else {
                    binding.faqs.setText(Html.fromHtml(list.get(i).getFaq()));
                }
//                binding.faqs.setText(list.get(i).getFaq());
            }
        }
    }
}