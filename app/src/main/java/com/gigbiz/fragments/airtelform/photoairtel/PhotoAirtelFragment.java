package com.gigbiz.fragments.airtelform.photoairtel;
...
import com.gigbiz.databinding.FragmentPhotoAirtelBinding;
...
public class PhotoAirtelFragment extends Fragment {

    private PhotoAirtelViewModel mViewModel;
    FragmentPhotoAirtelBinding binding;
    File trans_compressed;
    File qr_code_compressed;
    SharedPreferences sh;
    private FormDashBoardActivity formDashBoardActivity;
    private boolean check = false;
    String report_id;

    public static PhotoAirtelFragment newInstance() {
        return new PhotoAirtelFragment();
    }

    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
                             @Nullable Bundle savedInstanceState) {
        binding = FragmentPhotoAirtelBinding.inflate(inflater, container, false);
...
//                }
                if (trans_compressed == null){
                    Toast.makeText(getContext(), "please select image", Toast.LENGTH_SHORT).show();
                }else if (qr_code_compressed == null){
                    Toast.makeText(getContext(), "please select image", Toast.LENGTH_SHORT).show();
                }
                else {
...
//        }
    ...
    private void sumbitSecondReport() {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                Call<SubmitAirtelReportSecondResponse> call = RetrofitClient.getInstance()
                        .getApi().getAirtelReportSecondResponse(
                                new SubmitAirtelReportSecondRequest(
                                        MainHelper.getUserDataList(sh).get(0).getUserId(),
                                        MainHelper.getUserDataList(sh).get(0).getToken(),
                                        report_id
                                        ,MainHelper.getFileToByte(trans_compressed.getAbsolutePath())
                                        ,MainHelper.getFileToByte(qr_code_compressed.getAbsolutePath())
                                        ,MainHelper.getUserDataList(sh).get(0).getType().toLowerCase()
                                ));

                call.enqueue(new Callback<SubmitAirtelReportSecondResponse>() {
                    @Override
                    public void onResponse(Call<SubmitAirtelReportSecondResponse> call, Response<SubmitAirtelReportSecondResponse> response) {
                        SubmitAirtelReportSecondResponse secondResponse = response.body();
...
//                            startActivity(new Intent(LoginActivity.this, HomeActivity.class));
                    ...
                    @Override
                    public void onFailure(Call<SubmitAirtelReportSecondResponse> call, Throwable t) {
                        try {
            ...
            if (resultCode == RESULT_OK) {
                ...
                if (requestCode == 1000) {
                    if (null != selectedImageUri) {
                        ...
                        trans_compressed = new File(MainHelper.getRealPathFromURI(getActivity(),selectedImageUri));
                        Glide.with(getActivity())
                                .load(trans_compressed.getAbsolutePath())
                                .into(binding.transactionImg);
                        Toast.makeText(getContext(), trans_compressed.getAbsolutePath(), Toast.LENGTH_SHORT).show();

                    }
                }else if (requestCode == 2000) {
                    if (null != selectedImageUri) {

                        qr_code_compressed = new File(MainHelper.getRealPathFromURI(getActivity(),selectedImageUri));
                        Glide.with(getActivity())
                                .load(qr_code_compressed.getAbsolutePath())
                                .into(binding.qrCodeImg);
                        Toast.makeText(getContext(), qr_code_compressed.getAbsolutePath(), Toast.LENGTH_SHORT).show();
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        mViewModel = new ViewModelProvider(this).get(PhotoAirtelViewModel.class);
